...
#include <console.h>
#include <vmm.h>
#include <pmm.h>  // 2310675: COW Challenge - 需要do_cow_page_fault函数
#include <kdebug.h>
#include <unistd.h>
#include <syscall.h>
#include <error.h>
...
void exception_handler(struct trapframe *tf)
{
    ...
    case CAUSE_BREAKPOINT:
        cprintf("Breakpoint\n");
        // 笔记: 通过ebreak指令产生断点中断，实现在内核态使用系统调用
        // 笔记: 用a7寄存器值为10标识这是系统调用而非普通断点
        if (tf->gpr.a7 == 10)
        {
            tf->epc += 4;  // 笔记: 返回时执行ebreak的下一条指令
            syscall();
            // 笔记: kernel_execve_ret调整trapframe位置，实现从内核态到用户态的切换
    ...
    case CAUSE_FETCH_PAGE_FAULT:
        ...
        // 2310675: 指令页面错误通常是非法访问，应终止进程
        if ((tf->status & SSTATUS_SPP) == 0) {
            // 2310675: 来自用户态的page fault，标记进程退出
            do_exit(-E_KILLED);
        }
        break;
    case CAUSE_LOAD_PAGE_FAULT:
        cprintf("Load page fault\n");
        // 2310675: 加载页面错误，如果来自用户态则终止进程
        if ((tf->status & SSTATUS_SPP) == 0) {
            do_exit(-E_KILLED);
        }
        break;
    case CAUSE_STORE_PAGE_FAULT:
        // 2310675: COW Challenge - Store page fault可能是写COW页面导致
        if ((tf->status & SSTATUS_SPP) == 0) {
            // 2310675: 来自用户态的page fault
            // 首先尝试COW处理
            uintptr_t fault_addr = read_csr(stval);  // stval存储触发fault的地址
            if (current != NULL && current->mm != NULL) {
                if (do_cow_page_fault(current->mm, tf->cause, fault_addr) == 0) {
                    // 2310675: COW处理成功，页面已复制，继续执行
                    break;
                }
            }
            // 2310675: 不是COW页面或处理失败，打印错误并终止进程
            cprintf("Store/AMO page fault at 0x%08x\n", fault_addr);
            do_exit(-E_KILLED);
        } else {
            // 2310675: 来自内核态的page fault，这是严重错误
